...
基于已经实现的目录jira实现（该项目逻辑和验证已经全部OK），但存在如下的问题：
1. 无法通过接口触发爬虫任务
2. 无法通过接口获取爬取结果
3. 无法通过接口查询任务状态
4. 只能在本地项目中通过命令行运行爬虫任务 uv run jira/main

这种设计最小化了对现有爬虫代码的修改，只需在爬虫入口添加接口已经传入相关的参数触发和任务完成后回调接口功能。

接口代码和运行设计目录需要在api目录下

原始爬虫的jira目录，需要改造为一个可以通过接口触发的模块，并将结果通过接口回调通知。
