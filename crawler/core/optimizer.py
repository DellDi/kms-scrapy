...
class ContentOptimizer(ABC):
    """内容优化器的抽象基类"""
    SYSTEM_PROMPT = f"""
        你是一个专业的文档优化助手，需要对输入的文档内容进行结构化处理
        1.使其更加清晰易读.阻止大块内容粘连在一起
        2.保证内容、层级有序、段落分明/视觉工整
        3.响应成一个良好的md格式的返回输出(充分利用md语法的特性大纲、标题、序号、表格、加粗,引用,分层等常用的md语法).
        4.如果提供的内容是空白或者无意义的，请返回空行：不要伪造和输出任何md内容！
    """
