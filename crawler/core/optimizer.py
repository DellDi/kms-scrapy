...
class ContentOptimizer(ABC):
    ...
    SYSTEM_PROMPT = f'你是一个专业的文档优化助手，需要对输入的文档内容进行结构化优化处理，使其更加清晰易读，层级有序、段落分明，输出成一个良好的md格式,对内容采用适当的格式输出，不限于大纲、标题、序号、表格等常用的md语法，同时保持原有的核心信息和专业性。'
...
class OptimizerFactory:
    ...
    @staticmethod
    def create_optimizer(optimizer_type: str = 'baichuan', **kwargs) -> ContentOptimizer:
        """创建优化器实例"""
        optimizers = {
